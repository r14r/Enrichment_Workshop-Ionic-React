...
function rootReducer(
	...
	switch (action.type) {
		case 'play':
			...
			state = {
				...state,
				queuePosition: action.payload.startIndex,
			};
			return state;
		...
		case 'playAlbum':
			toggleShuffle(
				state.musicKitInstance,
				action.payload.shouldShuffle
			);
			setQueueFromItems(
				state.musicKitInstance,
				action.payload.collection.relationships.tracks
					.data
			);
			return state;

		case 'togglePlay':
			state.musicKitInstance.player.playbackState ===
		...
		case 'seekToTime':
			state.musicKitInstance.player.seekToTime(
				action.payload
			);
			return state;
		...
		case 'playbackTimeDidChange':
			...
			return {
				...state,
				playbackProgress:
					action.payload.currentPlaybackTime,
			};
...
const setQueueFromItems = (
	...
	const newItems = items.map(
		(item) => (item.container = { id: item.id })
	);
	return musicKitInstance
		.setQueue({ items: newItems })
		.then(() =>
			changeQueuePosition(musicKitInstance, startPosition)
		);
};
...
const queueItemsDidChange = (state, payload) => {
	return {
		...state,
		queue: state.musicKitInstance.player.queue.items.slice(
			payload.position + 1
		),
	};
};
