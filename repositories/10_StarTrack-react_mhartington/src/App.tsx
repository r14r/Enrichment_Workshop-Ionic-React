...
function App() {
	...
	mkInstance.addEventListener(
		mkGlobal.Events.playbackStateDidChange,
		(e: any) => {
			dispatch({
	...
	mkInstance.addEventListener(
		mkGlobal.Events.queueItemsDidChange,
		(e: any) => {
			dispatch({ type: 'queueItemsDidChange', payload: e });
		}
	);
	mkInstance.addEventListener(
		mkGlobal.Events.mediaItemDidChange,
		(e: any) => {
			dispatch({ type: 'mediaItemDidChange', payload: e });
		}
	);
	mkInstance.addEventListener(
		mkGlobal.Events.queuePositionDidChange,
		(e: any) => {
			dispatch({
				type: 'queuePositionDidChange',
				payload: e,
			});
		}
	);
	mkInstance.addEventListener(
		mkGlobal.Events.playbackTimeDidChange,
		(e: any) => {
			dispatch({ type: 'playbackTimeDidChange', payload: e });
		}
	);
	mkInstance.addEventListener(
		mkGlobal.Events.playbackDurationDidChange,
		(e: any) => {
			dispatch({
				type: 'playbackDurationDidChange',
				payload: e,
			});
		}
	);
